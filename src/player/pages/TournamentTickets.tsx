...
import { ArrowLeft, Plus, Minus, ShoppingCart, Calendar, MapPin, Users, Ticket, Check, Star } from 'lucide-react';
...
export default function TournamentTickets() {
    ...
    const fetchTournament = async (tournamentId: string) => {
        ...
        try {
            ...
            // Fallback for missing ticket types (Dev/Demo fix)
            if (!data.ticketTypes || data.ticketTypes.length === 0) {
                const mock = MOCK_TOURNAMENTS.find(m =>
                    m.title.toLowerCase() === data.name.toLowerCase() ||
                    data.name.toLowerCase().includes(m.title.toLowerCase())
                );
                        ...
                        <div className="flex flex-col md:flex-row gap-6 md:items-end justify-between">
                            <div className="space-y-4 max-w-3xl animate-fade-in-up">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-primary font-bold tracking-widest uppercase">{tournament.gameId.title}</span>
                                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 -mt-8 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <div className="bg-[#1A1D21] border border-white/5 rounded-3xl p-8 backdrop-blur-sm">
                            ...
                            <div className="space-y-4">
                                {tournament.ticketTypes?.map((ticketType) => {
                                    ...
                                    return (
                                        <div
                                            key={ticketType.name}
                                            className={`relative bg-black/20 rounded-2xl p-6 transition-all duration-300 border ${hasQuantity
                                                ? isVIP
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 bg-[#1A1D21] border border-white/10 rounded-3xl p-6 shadow-2xl">
