...
import { LiveBroadcastPlaceholder, SeasonCard, StatPill } from './leagues/PlayerLeaguesSections';
...
export default function PlayerLeagues() {
    ...
    const [seasonRule, setSeasonRule] = useState<SeasonRule | null>(null);
    ...
    const inlineSeasonRule =
        selectedSeason?.rulesId && typeof selectedSeason.rulesId === 'object'
            ? selectedSeason.rulesId
            : null;
    const formatLabel = seasonRule
        ? `${seasonRule.formatType} · ${seasonRule.matchType}`
        : inlineSeasonRule
            ? `${inlineSeasonRule.formatType} · ${inlineSeasonRule.matchType}`
            : (selectedLeague as LeagueFull)?.format ?? '—';
    const maxTeamsLabel = seasonRule?.maxTeams ?? (selectedLeague as LeagueFull)?.maxTeams ?? '—';
    ...
    return (
        <div className="flex flex-col gap-4 h-full min-h-0 animate-fade-in-up">
            <div className="flex items-center gap-2 px-1">
                <Trophy className="w-4 h-4 text-primary" />
                <span className="text-[11px] font-black uppercase tracking-widest text-text-muted">All Leagues</span>
            </div>
            <div className="overflow-x-auto pb-1">
                <div className="flex items-stretch gap-2.5 min-w-max pr-1">
                    {leagues.map(league => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "w-[270px] text-left px-4 py-3.5 rounded-2xl border transition-all duration-200 group",
                    ...
                    <div className={cn("relative overflow-hidden rounded-3xl border border-white/10 p-6", lc.glow)} style={{ background: '#0f0f10' }}>
                        ...
                        <div className="relative flex flex-col lg:flex-row lg:items-start justify-between gap-4 w-full">
                            <div className="flex items-start gap-4 min-w-0 flex-1">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 flex-wrap">
                                        ...
                                        <StatPill icon={<Users size={11} />} label="Format" value={formatLabel} />
                                        <StatPill icon={<Shield size={11} />} label="Max Teams" value={`${maxTeamsLabel}`} />
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
                                <button
                                    type="button"
                                    onClick={() => navigate(`/player/leagues/${selectedLeague._id}/select-pass`)}
                        ...
                        <div className="flex-1 bg-[#0f0f10] border border-white/[0.07] rounded-3xl overflow-hidden flex flex-col min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-1.5">
                                {standingsLoading || (selectedSeasonId && matchesLoading && seasonStandings.length === 0) ? (
                                    ...
                                    const isTeam = !!team;
                                    const name = team?.name || p.playerId?.nickname || 'Unknown';
                                    const avatar = team?.logo || p.playerId?.avatar || null;
