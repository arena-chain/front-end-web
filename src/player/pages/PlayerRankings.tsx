...
type RankTier = 'CHALLENGER' | 'GRANDMASTER' | 'MASTER' | 'DIAMOND' | 'PLATINUM' | 'GOLD' | 'SILVER' | 'BRONZE' | 'IRON' | 'UNRANKED';
...
const TIER_CONFIG: Record<RankTier, { color: string; bg: string; border: string; emoji: string }> = {
    CHALLENGER: { color: '#ffd700', bg: 'rgba(255,215,0,0.1)', border: 'rgba(255,215,0,0.25)', emoji: '👑' },
    GRANDMASTER: { color: '#ff4655', bg: 'rgba(255,70,85,0.1)', border: 'rgba(255,70,85,0.25)', emoji: '💀' },
    MASTER: { color: '#f97316', bg: 'rgba(249,115,22,0.1)', border: 'rgba(249,115,22,0.25)', emoji: '⭐' },
    ...
    IRON: { color: '#64748b', bg: 'rgba(100,116,139,0.1)', border: 'rgba(100,116,139,0.25)', emoji: '⚙️' },
    UNRANKED: { color: '#475569', bg: 'rgba(71,85,105,0.1)', border: 'rgba(71,85,105,0.25)', emoji: '❔' },
};
...
export default function PlayerRankings() {
    ...
    useEffect(() => {
        const fetchPlayers = async () => {
            setLoading(true);
            try {
                // For "All Games", we can default to a primary game or total rankings
                // Let's assume gameId '000000000000000000000000' is a placeholder for global
                const gameId = game === 'All Games' ? '65f1a23b4f9e1c001f3d4e56' : '65f1a23b4f9e1c001f3d4e56'; // Replace with real game IDs
                const data = await rankingService.getLeaderboard(gameId);

                const mappedPlayers: Player[] = data.map((p: any) => ({
                    id: p._id,
                    name: p.user?.nickname || 'Unknown',
                    avatar: p.user?.avatar || 'Felix',
                    country: p.user?.country || 'Unknown',
                    countryCode: '🌍', // Dynamic lookup if available
                    region: (p.user?.region || 'EU') as Region,
                    tier: (p.tier || 'IRON') as RankTier,
                    elo: p.elo || 0,
                    winRate: p.winRate || 0,
                    wins: p.wins || 0,
                    losses: p.losses || 0,
                    kd: 1.0, // Backend might need to provide this
                    matches: p.totalMatches || 0,
                    trend: 'same',
                    trendDelta: 0,
                    isOnline: true,
                    isPro: false,
                    game: p.game?.title || 'Valorant'
                }));
    ...
    const filtered = useMemo(() => {
        return players
            .filter((p: Player) => region === 'ALL' || p.region === region)
            .filter((p: Player) => game === 'All Games' || p.game === game)
            .filter((p: Player) => tierFilter === 'ALL' || p.tier === tierFilter)
            .filter((p: Player) => p.name.toLowerCase().includes(search.toLowerCase()) || p.country.toLowerCase().includes(search.toLowerCase()))
            .sort((a: Player, b: Player) => b[sortBy] - a[sortBy]);
            ...
            <div className="relative overflow-hidden rounded-3xl shrink-0"
                ...
                <div className="relative px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-5">
                    ...
                    <div className="flex gap-3 shrink-0">
                        {(['CHALLENGER', 'GRANDMASTER', 'DIAMOND'] as RankTier[]).map(t => {
                            const tc = TIER_CONFIG[t] || TIER_CONFIG.UNRANKED;
                            const count = players.filter((p: Player) => p.tier === t).length;
                            return (
