...
const DEMO_MARKETPLACE: NftAvatar[] = [
    ...
    { _id: 'm3', name: 'Neon Fox', image: 'https://api.dicebear.com/7.x/bottts-neutral/svg?seed=neon', description: 'Fast and cunning digital fox', rarity: 'RARE', price: 150, listed: true, listPrice: 200, ownerId: { _id: 'o3', username: 'VoidHunter' }, createdAt: new Date().toISOString() },
...
export default function PlayerMarketplace() {
    ...
    const liveAuctions = marketplace.slice(0, 6);
    ...
    // Featured NFT (highest price)
    const featured = [...marketplace].sort((a, b) => (b.listPrice ?? b.price) - (a.listPrice ?? a.price))[0] ?? DEMO_MARKETPLACE[0];
                ...
                <div className="relative flex items-center gap-8 p-8 md:p-10">
                    ...
                    <div className="hidden md:flex shrink-0 w-48 h-48 rounded-2xl bg-black/30 border border-white/10 p-4 items-center justify-center relative">
                        <img
                            ...
                            onError={e => { (e.target as HTMLImageElement).src = `https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${featured.name}`; }}
                        />
                        <span className={`absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-lg text-[8px] font-black uppercase ${RARITY_STYLES[featured.rarity].badge}`}>
                            {RARITY_ICON[featured.rarity]} {featured.rarity}
                        </span>
                    </div>
