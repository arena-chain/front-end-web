...
import { getTeamId } from '../lib/parseAuthUser';
import { getStoredUserNormalized } from '../lib/session';
import { fetchTeamDetailForRoster } from './teamsPublic.service';
...
export const teamManagerService = {
    ...
    getMyTeam: async (): Promise<TeamData | null> => {
        const attachLocalStatus = (data: TeamData): TeamData => {
            const u = getStoredUserNormalized();
            const st = u?.teamManagerProfile?.status;
            if (st && !data.managerStatus) {
                return { ...data, managerStatus: st, managerVerified: st === 'approved' };
            }
            return data;
        };
        const fallbackFromStoredTeamId = async (): Promise<TeamData | null> => {
            const tid = getTeamId(getStoredUserNormalized());
            if (!tid) return null;
            const fb = await fetchTeamDetailForRoster(tid);
            if (!fb) return null;
            const u = getStoredUserNormalized();
            const st = u?.teamManagerProfile?.status ?? 'pending';
            return attachLocalStatus({
                _id: fb._id,
                name: fb.name,
                logo: fb.logo,
                ...
                members: fb.members,
                managerStatus: st,
                managerVerified: st === 'approved',
            });
        };
        try {
            const res = await axios.get(`${API_URL}/team-manager/me/team`, auth());
            return attachLocalStatus(res.data as TeamData);
        } catch (err: unknown) {
            const fb = await fallbackFromStoredTeamId();
            if (fb) return fb;
...
export interface TeamData {
    ...
    members: TeamMember[];
    isVerified?: boolean;
    /** `pending` until admin approves — invite/remove blocked server-side until `approved`. */
