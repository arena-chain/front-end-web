...
export const leagueService = {
    getAllLeagues: async (): Promise<League[]> => {
        const url = `http://127.0.0.1:3000/api/leagues`;
        console.log('leagueService: Fetching from:', url);
        try {
            const response = await axios.get(url, { headers: authHeader() });
            console.log('leagueService: Status:', response.status);
            console.log('leagueService: Raw data:', response.data);
            ...
            console.error('leagueService: Axios error:', e.message);
            if (e.response) {
                console.error('leagueService: Response data:', e.response.data);
                console.error('leagueService: Response status:', e.response.status);
            }
            throw e;
        }
    },
    ...
    registerForLeague: async (id: string, category?: string): Promise<any> => {
        const response = await axios.post(`${API_URL}/leagues/${id}/register`, { category }, { headers: authHeader() });
        return response.data;
    },
};
