        ...
        <div className="space-y-6">
            ...
            {showCreate && (
                <CreateNftModal
                    ...
                    onTicketCreated={(leagueId) => {
                        setShowCreate(false);
                        navigate('/admin/tickets', {
                            state: { selectedLeagueId: leagueId, openEditor: true, refreshNow: true },
                        });
                    }}
                />
            )}
...
function CreateNftModal({
    ...
    onTicketCreated: (leagueId: string) => void;
}) {
    ...
    useEffect(() => {
        const loadLeagues = async () => {
            try {
                ...
                console.log('NFT_MODAL: Leagues found count:', Array.isArray(data) ? data.length : 0);
                setLeagues(Array.isArray(data) ? data : []);
            } catch (error: any) {
                console.error('NFT_MODAL: Error loading leagues:', error.message);
                setLeagues([]);
            } finally {
    ...
    const handleSubmit = async () => {
        ...
        if (creationMode === 'ticket' && (!selectedLeagueId || !ticketName || vipCapacity <= 0 || mintQuantity <= 0)) return;
        try {
            setSaving(true);
            ...
            if (creationMode === 'ticket') {
                const league = await leagueService.getLeagueById(selectedLeagueId);
                const existing = Array.isArray((league as any)?.ticketTypes)
                    ? (league as any).ticketTypes
                    : [];
                ...
                await ticketService.addTicketTypesToLeague(selectedLeagueId, nextTicketTypes);
                const afterSaveLeague = await leagueService.getLeagueById(selectedLeagueId);
                const savedTypes = Array.isArray((afterSaveLeague as any)?.ticketTypes)
                    ? (afterSaveLeague as any).ticketTypes
                    : [];
                ...
                onTicketCreated(selectedLeagueId);
                return;
            }
        } catch (e) {
                    ...
                    <div className="rounded-2xl border border-lime-400/20 bg-gradient-to-br from-lime-400/10 via-black/40 to-violet-500/10 p-4">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p className="text-[11px] text-white/50 mt-1">{selectedLeague?.name || 'Select a league'}</p>
                            </div>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="border border-lime-400/20 rounded-2xl p-4 bg-black/40 space-y-3">
                            <div className="flex items-center justify-between">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-red-500">VIP Ticket Configuration (V2)</p>
                                <span className="text-[9px] font-bold uppercase tracking-widest text-white/40">Admin Secure Node</span>
                            </div>
                                <div className="flex items-center justify-between mb-1.5">
                                    <label className="block text-[10px] font-bold text-text-muted uppercase tracking-widest">League</label>
                                    <button 
                                        type="button"
                                        onClick={() => {
                                            const loadLeagues = async () => {
                                        ...
                                        className="text-[9px] font-bold text-lime-400 hover:text-white transition-colors uppercase tracking-widest"
                                    >
                                        [ Refresh List ]
                                    </button>
                                </div>
                                ...
                                {loadingLeagues && (
                                    <p className="text-[10px] text-text-muted mt-1 animate-pulse">Connecting to backend...</p>
                                )}
                            <div>
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <button
                        ...
                        disabled={
                            ...
                            || (creationMode === 'ticket' && (!selectedLeagueId || !ticketName || vipCapacity <= 0 || mintQuantity <= 0))
                        }
