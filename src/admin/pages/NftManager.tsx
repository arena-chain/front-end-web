...
export default function NftManager() {
    ...
    const [activeView, setActiveView] = useState<'studio' | 'inventory' | 'marketplace'>('studio');
    ...
    const load = async () => {
        try {
            ...
            const [nftData, colData, mktData, statsData] = await Promise.all([
                ...
                <div className="bg-surface border border-white/5 rounded-2xl overflow-hidden">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-white/5">
                            <tr>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-text-muted leading-none">Item</th>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-text-muted leading-none">Owner</th>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-text-muted leading-none">Price</th>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-text-muted leading-none">Moderation</th>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-text-muted leading-none text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {marketplaceItems.map(item => (
                                <tr key={item._id} className="hover:bg-white/[0.02] transition-colors group/row">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-lg overflow-hidden bg-black/40 border border-white/10 group-hover/row:border-primary/30 transition-all">
                                                <img src={getImageUrl((item.nftId as any)?.imageUrl)} className="w-full h-full object-cover" alt="" />
                                            </div>
                                            <div>
                                                <p className="text-white text-sm font-black uppercase tracking-tight">{(item.nftId as any)?.name}</p>
                                                <p className="text-[10px] text-text-muted font-bold uppercase">{(item.nftId as any)?.rarity}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className="text-white text-xs font-bold">{(item.ownerId as any)?.nickname || 'Unknown'}</p>
                                        <p className="text-[10px] text-text-muted font-medium truncate max-w-[120px]">{(item.ownerId as any)?.email}</p>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1.5">
                                            <span className="text-primary font-black text-sm">{item.listPrice}</span>
                                            <span className="text-[10px] text-text-muted font-black">AC</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        {item.isFeatured ? (
                                            <span className="flex items-center gap-1.5 text-[10px] bg-primary/20 text-primary px-3 py-1 rounded-full font-black uppercase tracking-widest w-fit animate-pulse">
                                                <Sparkles size={10} fill="currentColor" /> Featured
                                            </span>
                                        ) : (
                                            <span className="text-[10px] text-text-muted/40 font-black uppercase tracking-widest">Standard</span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                ...
                                                className={`p-2 rounded-lg transition-all ${item.isFeatured ? 'bg-primary text-black shadow-[0_0_15px_rgba(0,255,136,0.3)]' : 'bg-white/5 text-text-muted hover:text-white hover:bg-white/10'}`}
                                                title={item.isFeatured ? 'Unfeature' : 'Feature on Marketplace'}
                                            >
                                                <Sparkles size={14} />
                                            </button>
                                            <button
                                                onClick={() => handleForceUnlist(item._id)}
                                ...
                                <tr>
                                    <td colSpan={5} className="py-20 text-center">
                                        <div className="flex flex-col items-center gap-2 opacity-20">
                                            <Box size={40} />
                                            <p className="text-sm font-black uppercase tracking-widest">No active listings</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                    ...
                    <div className="grid grid-cols-3 gap-1">
                        {(['front', 'right', 'back', 'left', 'top', 'bottom'] as CameraPreset[]).map((preset) => (
                            <button
                                ...
                                className={`rounded-md border px-2 py-1 text-[9px] font-black uppercase tracking-widest transition-all ${activePreset === preset
